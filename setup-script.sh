    ...
    case "$project_type" in
        laravel)
            if [ -n "$repo_link" ]; then
                ...
                ask_for_input "No GitHub link provided. Choose setup option: (1) Plain Laravel, (2) Custom Laravel 11 template, (3) Nothing: " setup_option
                ...
                case "$setup_option" in
                    ...
                    2)
                        ...
                        git clone https://github.com/BlackSparowYT/laravel11-template "$domain_dir"
                        cd "$domain_dir" || exit
                    ...
                    3)
                        ...
                        echo -e "\e[33m5. The script will proceed in 10 sec to generate an nginx config file and try to set up SSL.\e[0m"
                        sleep 10
                        ;;
                    *)
        ...
        react)
            if [ -n "$repo_link" ]; then
                # Install React dependencies from GitHub
                npm install
                npm run build
                ...
                # Ask if user wants a default React setup
                ...
                if [[ "$setup_default" == "yes" || "$setup_default" == "y" ]]; then
                    ...
                    echo -e "\e[33m3. The script will proceed in 10 seconds to generate an Nginx config file and try to set up SSL.\e[0m"
                    sleep 10
                fi
            fi
            ;;
        ...
        docker)
            ask_for_input "Enter the port the container is reachable on: " port
            ;;
        
        plain)
            # No special setup for plain/static sites
            ;;
    esac
fi
